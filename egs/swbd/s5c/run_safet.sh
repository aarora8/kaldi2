...
if [ $stage -le 2 ]; then
  ...
  mkdir -p data/ihm
  utils/copy_data_dir.sh data/train data/ihm/train_swbd
fi
...
if [ $stage -le 1 ]; then
  #local/safet_get_cmu_dict.sh
  utils/prepare_lang.sh data/local/dict_nosp_final '<UNK>' data/local/lang_nosp data/lang_nosp
...
if [ $stage -le 3 ]; then
  for dset in train; do
    ...
    [ "$mic" == "ihm" ] && seconds_per_spk_max=120
...
if [ $stage -le 5 ]; then
  ...
  (
    local/safet_cleanup_transcripts.py data/local_safet/lexicon.txt data/safe_t_r11/transcripts data/safe_t_r11/transcripts.clean
    local/safet_cleanup_transcripts.py data/local_safet/lexicon.txt data/safe_t_r20/transcripts data/safe_t_r20/transcripts.clean
  ) | sort > exp/cleanup_stage_1/oovs

  local/safet_cleanup_transcripts.py --no-unk-replace  data/local_safet/lexicon.txt \
...
if [ $stage -le 6 ] ; then
  ...
  utils/format_lm.sh  data/lang_nosp/ data/local/srilm/lm.gz\
    data/local/dict_nosp_final/lexicon.txt  data/lang_nosp_test
fi
...
if [ $stage -le 9 ]; then
  utils/copy_data_dir.sh /export/c05/aarora8/kaldi2/egs/ami/s5b/data/ihm/train data/ihm/train_ami
