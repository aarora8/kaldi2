...
tdnn_affix=1a_lr001_noise  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 20 ]; then
    steps/nnet3/decode.sh --num-threads 4 --nj 20 --cmd "$decode_cmd" \
        ...
        --online-ivector-dir exp/nnet3${nnet3_affix}/ivectors_safe_t_dev1_hires_noise \
       $dir/graph data/safe_t_dev1_hires $dir/decode_safe_t_dev1_noise || exit 1;
fi
exit 0
