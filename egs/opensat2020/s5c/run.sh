...
if [ $stage -le 4 ]; then
  ...
  utils/subword/prepare_lang_subword.sh data/local/dict "<UNK>" data/local/lang data/lang_nosp

  for data_set in train safe_t_dev1; do
    cut -d ' ' -f2- data/$data_set/text |  utils/lang/bpe/apply_bpe.py -c data/local/pair_code.txt --glossaries "<UNK>" > data/$data_set/text.sub
    mv data/$data_set/text data/$data_set/text.old
    cut -d ' ' -f1 data/$data_set/text.old | paste -d ' ' - data/$data_set/text.sub > data/$data_set/text
  done
