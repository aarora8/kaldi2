...
gmm_dir=exp/$mic/${gmm}
ali_dir=exp/$mic/${gmm}_ali_train_ali_aug
...
#
#  num_utts_total=$(wc -l <data/$mic/${train_set}_hires/utt2spk)
#  num_utts=$(($num_utts_total/4))
#  utils/data/subset_data_dir.sh data/$mic/${train_set}_hires \
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 2 \
     data/$mic/${train_set}_hires exp/$mic/nnet3${nnet3_affix}/diag_ubm \
...
if [ $stage -le 6 ]; then
  ...
  ivectordir=exp/$mic/nnet3${nnet3_affix}/ivectors_${train_set}_hires
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/$mic/${train_set}_hires ${temp_data_root}/${train_set}_hires_max2
