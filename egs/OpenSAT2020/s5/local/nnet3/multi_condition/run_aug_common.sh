...
if [ $stage -le 1 ]; then
  ...
  steps/data/augment_data_dir.py --utt-prefix "noise" --modify-spk-id "true" \
    ...
    data/${clean_set_aug} data/${clean_set_aug}_noise
  steps/data/augment_data_dir.py --utt-prefix "music" --modify-spk-id "true" \
    ...
    data/${clean_set_aug} data/${clean_set_aug}_music
  steps/data/augment_data_dir.py --utt-prefix "babble" --modify-spk-id "true" \
    ...
    data/${clean_set_aug} data/${clean_set_aug}_babble

  combine_str=""
  for n in $aug_list; do
    if [ "$n" == "clean" ]; then
      # clean refers to original of training directory
      combine_str+="data/$clean_set_aug "
    else
      combine_str+="data/${clean_set_aug}_${n} "
    fi
  done
  utils/combine_data.sh data/${clean_set_aug}_aug $combine_str
fi
